$(document).ready(function () { /*code here*/
    ...
    function loaddata(data) {
        ...
        var inorigin = document.getElementById("from").value;
        var indestination = document.getElementById("to").value;

        
        // alert(" sess " + useremail + " " + inorigin + " " + indestination);
        

        //alert("in load");
        for (i = 0; i < data.length; i++) {
            //data[0]["Id"]
            //alert(data[i]["Origin"] + " " + inorigin);

            if ((data[i]["Origin"] == inorigin && indestination == "") ||
                 (data[i]["Destination"] == indestination && inorigin == "") ||
                 (data[i]["Destination"] == indestination && data[i]["Origin"] == inorigin)
               ) {
                ...
                document.getElementById("searchcarpools").appendChild(div1);
            }
        }
    }
    ...
    function loadArrays(data) {
        ...
        for (k = 0; k < data.length; k++) {
            origin[k] = data[k]["Origin"];
            destination[k] = data[k]["Destination"];
        }
